...
function 配置信息(密码, 域名地址) {
	...
	const v2ray = `${协议类型}://${encodeURIComponent(密码)}@${地址}:${端口}?security=${传输层安全[0]}&sni=${SNI}&alpn=h3&fp=${指纹}&allowInsecure=1&type=${传输层协议}&host=${encodeURIComponent(路径)}#${encodeURIComponent(别名)}`              
